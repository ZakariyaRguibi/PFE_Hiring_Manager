...
export default class CandidateJobList extends LightningElement {
    ...
    get hasNoJobs() {
        return !this.isLoading && !this.error && (!this.jobs || this.jobs.length === 0);
    }
    ...
    get isNextDisabled() {
        return this.isUploadingFiles || this.isParsingCv || this.isSubmittingApplication;
    }
    ...
    handleViewDetails(event) {
        ...
        if (!this.selectedJobId) {
            alert('Unable to identify the selected job. Please refresh the page and try again.');
            return;
        }
    ...
    async handleApply() {
        ...
        if (!this.selectedJobId) {
            alert('Job position is missing. Please close this popup and select the job again.');
            return;
        }
        ...
        try {
            ...
            console.error('Unable to load candidate profile/files:', JSON.stringify(error));
            this.showAuthMessage = true;
        }
    }
    ...
    async handleNext() {
        ...
        try {
            ...
            if (this.motivationLetterMode === 'existing') {
            ...
            } else if (this.motivationLetterFile && !this.motivationLetterContentDocumentId) {
                const motivationBase64 = await this.readFileAsBase64(this.motivationLetterFile);
                ...
                this.motivationLetterContentDocumentId = motivationResult.contentDocumentId;
            }
            ...
            this.uploadMessage = 'CV analyzed successfully. Please review the extracted information.';
    ...
    async handleFinalApply() {
        ...
        try {
            const requestPayload = {
                ...
                motivationLetterContentDocumentId: this.motivationLetterContentDocumentId,
